...
и специальных символов (см. ниже исключение Ь).
Специальный символ не имеет собственного значения.
В итоге, ни одна мультиграфема не состоит из комбинации символов, 
каждая из которых может быть преобразована по отдельности
(например, 
недопустимо представление Ц как TS (Статский, Высоцкий),
Щ как СЧ SCH (Полосчатый) или ШЧ SHCH (Веснушчатый)
...
что исключает неоднозначность в ситуации, когда конечная часть мультиграфемы может быть воспринята как начальная часть следующей;
плюс обеспечит удобство посимвольной обработки в один проход (при необходимости)
...
RULES = {
    ...
    # при модификации словаря вручную можно использовать как заглавный, так и строчный регистр
    ...
    ,'Ь' : 'JH' # этот (очень короткое И) смягчал, поэтому его Й (символизирующий краткость) ослабляем немым H
                # единственная комбинация двух специальных символов, но они расположены в правильном порядке
    ...
    ,'Г' : 'G'
    ,'Д' : 'D'
    ,'Е' : 'E' # Е по идее, йотированный, но очень частый, не нужно еще больше J в тексте
    ,'З' : 'Z'
    ,'И' : 'I'
    ,'Й' : 'Y' # Й здесь
    ,'К' : 'K'
    ...
    ,'У' : 'U'
    ,'Ф' : 'F'
    ,'Ц' : 'C' # Как ни странно, в исходном латинском языке изначально читалась как К
...
# словарь для преобразования в транслит
RULES_IN = {}
for key in RULES:
...
def silly_trans_from(inStr):
    ...
    """Для исправления такого битого преобразования нужно будет битую строку преобразовать обратно в транслит,
и применить к транслиту уже корректную функцию
